...
#if defined(CONFIG_INPUT_BUTTONS) && !defined(ESP32C3_GPIO_IRQ)
...
uint32_t board_button_initialize(void)
{
  ...
  for (i = 0; i < NUM_BUTTONS; i++)
    {
      esp32c3_configgpio(g_buttons[i]);
    }
  ...
  for (i = 0; i < NUM_BUTTONS; i++)
    {
      ...
      bool released = esp32c3_gpioread(g_buttons[i]);
...
int board_button_irq(int id, xcpt_t irqhandler, void *arg)
{
  ...
  if (id >= MIN_IRQBUTTON && id <= MAX_IRQBUTTON)
    {
      ret = esp32c3_gpiosetevent(g_buttons[id], true, true, true, irqhandler,
                               arg);
    }
